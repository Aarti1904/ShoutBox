                 ...
                 @foreach($reports as $report)
                    ...
                    <div class="col-md-4" style="margin-top:20px">
                       <div class="card" style="width: 19rem;">
                          ...
                          <img class="card-img-top" class="card-img-top" style=" width: 100%; height: 15vw;" src="{{ $report->file}}">
                          @elseif($report->type == 'video')
                          <video class="card-img-top" style=" width: 100%; height: 15vw;"
                           id="myVideo"                           
                           loop
                           controls
                           muted
                          >
                          <source src="{{$report->file}}" />
                          </video>
                          @elseif ($report->type  == 'audio')
                             <div style=" width: 100%; height: 15vw; padding-top:80px;background-color:black"> <audio id="myAudio" controls>
                               <source  src="{{$report->file}}" >                              
                              </audio>
                              </div>

                              @elseif ($report->type =='text')
                              <div style=" width: 100%; height: 15vw; padding-top:20px;background-color:black"> 
                              <?php
                              $destinationPath = substr($report->file,21);
                          ...
                          @endif
                           <div class="card-body">
                               ...
                               <button class="btn btn" style="width:90px; background-color:#5FCF80" onclick="ignore(`{{$report->shoutId}}`)">Ignore</button>
                            </div>
                        </div>
                    </div>          
...
$.ajax({
type: 'GET',
url: "http://127.0.0.1:8000/ignoreShout/" + shoutId,
...
success: function (results) {
    ...
    {
        url="/report/destroy/"+results[i].id;
        reason+="<li>";
       reason+=(results[i].reportDescription);
       reason+=`&nbsp; <a onclick=deleteReport(`+results[i].id+`) style='color:red'>Delete</a><br>`;
       reason+="</li>";
     }
     reason+="</ul>"
    if (reason) {
